...
public class PhoneController {
	...
	public String doList(Phone phone, Model model) {
		try {
			...
			model.addAttribute("message", "문제 내용 - 폰 목록 조회 중 문제 발생");
			return "Error";
		}
	}
	...
	public String doDetail(Phone phone, Model model) {
		try {
			...
			model.addAttribute("title", "핸드폰 관리 - 에러");
			model.addAttribute("message", "문제 내용 - 폰 상세조회 중 문제 발생");
			return "Error";
		}
	}
	...
	public String doLogin(UserInfo user, Model model, HttpSession session) {
		// request도 써야할 경우에는 session 대신 request 받아오면 됨
		try {
			UserInfo selected = phoneService.select(user);
			if(selected!=null) {	// 찾는 계정이 존재할 경우
			...
			model.addAttribute("title", "회원 로그인 - 에러");
